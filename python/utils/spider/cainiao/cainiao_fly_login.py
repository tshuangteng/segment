...
from utils.chromium import set_user_agent, intercept_request, read_token, get_cookies, generate_headers
...
def get_headers_and_cookies(username, password):
    user_agent, referer, fly_cookies = '', '', ''

    while not fly_cookies:
        user_agent, referer, fly_cookies = asyncio.get_event_loop().run_until_complete(
            get_cookies_and_referer_for_link(username, password))
    fly_headers = generate_headers(user_agent, referer)

    return fly_headers, fly_cookies
